          ...
          dateEntry.shows.forEach((show) => {
            ...
            if (theaterName) {
              ...
              dateView[date].theaters[theaterName].rooms[
                ...
                time: show.time,
                endTime: calculateEndTime(show.time, movie.duration),
                movieId: movie.id,
  ...
  const roomViewArray = Object.entries(roomView).map(
    ([theaterName, theater]) => ({
      ...
      rooms: theaterStructure[theaterName].rooms
        ...
        .map((roomName) => ({
          name: roomName,
          dates: Object.values(theater.rooms[roomName].dates).sort((a, b) =>
            a.date.localeCompare(b.date)
        ...
        if (dateEntry.shows && dateEntry.shows.length > 0) {
          ...
          dateEntry.shows.forEach((show) => {
            ...
            if (theaterName) {
              ...
              if (
                !movieView[movie.id].dates[date].theaters[theaterName].rooms[
                  roomName
                ]
              ) {
                movieView[movie.id].dates[date].theaters[theaterName].rooms[
